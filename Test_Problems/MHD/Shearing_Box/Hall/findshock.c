...
/* ----------------------------------------------
  ...
  for (i = ibeg; i <= iend; i++){
    ...
    // New flag to stabilise low beta plasmas
      
    D_EXPAND(beta = 0.5*bx[k][j][i]*bx[k][j][i];   ,
            beta += 0.5*by[k][j][i]*by[k][j][i];   ,
            beta += 0.5*bz[k][j][i]*bz[k][j][i];)
