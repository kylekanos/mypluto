...
/*! 
  ...
  For this reason, in 2D pherical cordinates we swap the role of the  
  "y" and "z" coordinates.
...
/* ********************************************************************* */
void WriteVTK_Vector (FILE *fvtk, Data_Arr V, 
...
/* --------------------------------------------------------
  ...
  if (vel_field || mag_field) { 
    DOM_LOOP(k,j,i){ 
      D_EXPAND(v1 = V[0][k][j][i]; x1 = grid[IDIR].x[i]; ,
               v2 = V[1][k][j][i]; x2 = grid[JDIR].x[j]; ,
               v3 = V[2][k][j][i]; x3 = grid[KDIR].x[k];)
       ...
       #if DIMENSIONS == 2
        vect3D[k][j][i].v1 = v1*sin(x2) + v2*cos(x2);
...
/* ********************************************************************* */
void WriteVTK_Scalar (FILE *fvtk, double ***V, 
                      char *var_name, Grid *grid)
/*!
 ...
 *********************************************************************** */
{
  ...
  Vflt = Convert_dbl2flt(V, IsLittleEndian());
